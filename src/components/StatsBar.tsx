      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {STATS.map((stat, idx) => (
            <div
              key={stat.id}
              className="rounded-2xl bg-[#141C18] border border-white/10 p-7 sm:p-8 flex flex-col justify-between hover:border-[#D4AF37]/50 transition-all group relative overflow-hidden"
            >
              <div className="flex items-center justify-between mb-6">
                ...
                <span className="p-2 rounded-lg bg-[#1B4332]/50 border border-white/10">
                  {getIcon(stat.iconName)}
                </span>
              </div>

              <div>
                <div className="font-serif text-4xl sm:text-5xl lg:text-6xl font-bold text-white tracking-tight mb-2 group-hover:text-[#D4AF37] transition-colors">
                  {stat.value}
                </div>
