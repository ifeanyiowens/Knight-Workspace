...
export const ServicesPage: React.FC<ServicesPageProps> = ({ onOpenBooking, onNavigate }) => {
  ...
  const getServiceIcon = (iconName: string) => {
    switch (iconName) {
      case 'FileCode2':
        return <FileCode2 className="w-5 h-5 text-[#04703D]" />;
      case 'Workflow':
        return <Workflow className="w-5 h-5 text-[#04703D]" />;
      case 'Database':
        return <Database className="w-5 h-5 text-[#04703D]" />;
      case 'Layers':
        return <Layers className="w-5 h-5 text-[#04703D]" />;
      case 'Sparkles':
        return <Sparkles className="w-5 h-5 text-[#04703D]" />;
      default:
        return <Zap className="w-5 h-5 text-[#04703D]" />;
    }
  };
    ...
    <div className="w-full text-[#0A0A0A]">
      ...
      <section className="py-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 pb-10 mb-12 border-b border-black/10">
          ...
          <p className="text-xs font-mono text-[#0A0A0A]/60 max-w-sm">
        ...
        <div className="rounded-3xl bg-white border border-black/10 p-8 sm:p-10 shadow-2xl mb-20 relative overflow-hidden">
          ...
          <h3 className="font-serif text-3xl font-bold text-[#0A0A0A] mb-4">
            How we implement {selectedService.title}
          </h3>

          <p className="text-sm text-[#0A0A0A]/70 max-w-3xl leading-relaxed mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 pt-6 border-t border-black/10 mb-8">
            ...
            <div className="bg-white p-6 rounded-2xl border border-black/5 space-y-4">
              <h4 className="font-mono text-xs uppercase tracking-widest text-[#0A0A0A]/40 font-bold">
                IDEAL CLIENT FIT
              </h4>
              <p className="text-xs text-[#0A0A0A]/80 leading-relaxed font-mono">
                {selectedService.bestFor}
              </p>
              
              <div className="pt-4 border-t border-black/10">
                <span className="font-mono text-[10px] text-[#0A0A0A]/40 uppercase block mb-2">
                  TOOLS & INTEGRATIONS UTILIZED
                </span>
                <div className="flex flex-wrap gap-2">
                  {selectedService.tools.map((t) => (
                    <span key={t} className="px-3 py-1 rounded-lg bg-black/5 text-[#04703D] font-mono text-xs border border-black/10">
                      {t}
                    </span>
                  ))}
                </div>
          ...
          <div className="flex justify-end">
            <button
              ...
              className="px-6 py-3.5 rounded-xl bg-[#04703D] text-white font-mono text-xs font-bold uppercase tracking-wider hover:bg-[#059C54] transition-all flex items-center gap-2 cursor-pointer shadow-lg"
            >
              <Calendar className="w-4 h-4" />
              <span>Book a call</span>
            </button>
        ...
        <div className="rounded-3xl bg-white border border-black/10 p-8 sm:p-10 shadow-2xl mb-16">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h3 className="font-serif text-3xl sm:text-4xl font-bold text-[#0A0A0A] mb-3">
              Pick the Scope That Fits Right Now
            </h3>
            <p className="text-xs sm:text-sm text-[#0A0A0A]/60 font-sans">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
            ...
            <div className="p-7 rounded-2xl bg-white border border-black/10 flex flex-col justify-between hover-lift h-full">
              <div>
                ...
                <div className="font-mono text-xs text-[#0A0A0A]/50 mb-1">1 to 2 Days</div>
                ...
                <div className="space-y-2.5 text-xs text-[#0A0A0A]/80 mb-6 font-mono">
              ...
              <button
                ...
                className="w-full py-3 rounded-xl bg-black/5 hover:bg-black/10 text-[#0A0A0A] font-mono text-xs uppercase tracking-wider border border-black/10 transition-all cursor-pointer"
              >
                Book a call
              </button>
            </div>
            ...
            <div className="p-7 rounded-2xl bg-white border border-black/10 flex flex-col justify-between hover-lift h-full">
              <div>
                ...
                <div className="font-mono text-xs text-[#0A0A0A]/50 mb-1">2 to 3 Days</div>
            ...
            <div className="p-7 rounded-2xl bg-white border border-black/10 flex flex-col justify-between hover-lift h-full">
              <div>
                ...
                <div className="font-mono text-xs text-[#0A0A0A]/50 mb-1">3 to 4 Days</div>
          ...
          <div className="mt-6 p-5 rounded-2xl bg-black/5 border border-black/10 text-xs sm:text-sm text-[#0A0A0A]/70 text-center">
